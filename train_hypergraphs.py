...
def parse_args():
    # PARAMETERS
    parser = argparse.ArgumentParser('Hypergraphs')
...
def main(args):
    ...
    # 实例化模型
    m = HypergraphInpainting(in_ch=6, target_size=(80,80)).to(device)

    # 多GPU板卡
    # m = FHypergraphInpainting(in_ch=6, target_size=(80,80))
    #
    ...
    info = ''
    h = History('Hypergraphs', './history_save', args.learning_rate, args.epoch, args.batch_size, info=info)
    ...
    for epoch in range(1, 201):
        ...
        if test_log['psnr'] > best_psnr:
            ...
            torch.save(m.cpu().state_dict(), './model_save/Hypergraphs_best.pth'.format(info))
        ...
        torch.save(m.cpu().state_dict(), './model_save/Hypergraphs_last.pth'.format(info))
        m = m.to(device)
