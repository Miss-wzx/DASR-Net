...
def main(args):
    ...
    device = torch.device("cuda:2" if torch.cuda.is_available() and torch.cuda.device_count() >= 3 else "cpu")
    ...
    for epoch in range(1, 201):
        ...
        if test_log['psnr'] > best_psnr:
            ...
            torch.save(m.cpu().state_dict(), './model_save/CycleGAN_best.pth'.format(info))
        ...
        torch.save(m.cpu().state_dict(), './model_save/CycleGAN_last.pth'.format(info))
        m = m.to(device)
